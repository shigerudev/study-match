...
export default function ClassesPage() {
  return (
    <main className="min-h-screen bg-[#F3F4F6] px-4 py-7 pb-28 text-[#1F2937]">
      <section className="mx-auto max-w-[480px]">
        <Link className="text-sm font-semibold text-[#3B82F6]" href="/home">← Inicio</Link>
        ...
        <div className="mt-7 space-y-3">
          {subjects.filter((subject) => subject !== "Todo").map((subject) => {
            ...
            return (
              <Link
                key={subject}
                href={`/home?subject=${encodeURIComponent(subject)}`}
                className="flex items-center gap-4 rounded-2xl bg-white p-4 ring-1 ring-black/5 transition hover:bg-[#DBEAFE]"
              >
