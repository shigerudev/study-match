...
import { addFeedPost, subjects, type Subject } from "@/data/feed";

const classOptions = subjects.filter((subject): subject is Exclude<Subject, "Todo"> => subject !== "Todo");

export default function CreatePostPage() {
  const router = useRouter();
  const [type, setType] = useState<"foto" | "video">("foto");
  const [title, setTitle] = useState("");
  const [subject, setSubject] = useState<Exclude<Subject, "Todo"> | "">("");
  ...
  return (
    <main className="min-h-screen bg-[#F3F4F6] px-5 py-8 pb-28 text-[#1F2937] sm:py-12">
      <section className="mx-auto max-w-xl">
        <Link className="text-sm font-semibold text-[#3B82F6]" href="/home">
          ← Volver a Inicio
        </Link>
        ...
        {published ? (
          <div className="mt-8 rounded-3xl bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
            ...
            <p className="mt-2 text-sm leading-6 text-[#1F2937]/60">
              Se añadió al inicio del feed local de demostración.
            </p>
            <button
              type="button"
              onClick={() => router.push("/home")}
              className="mt-6 inline-flex rounded-full bg-[#3B82F6] px-5 py-3 text-sm font-semibold text-white"
            >
              Ver el feed
            </button>
          </div>
          ...
          <form className="mt-8 space-y-5 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-black/5" onSubmit={publish}>
            <fieldset>
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                <button
                  className={`flex items-center justify-center gap-2 rounded-2xl border px-4 py-4 text-sm font-semibold ${type === "foto" ? "border-[#3B82F6] bg-[#DBEAFE] text-[#3B82F6]" : "border-black/10 text-[#1F2937]/60"}`}
                  type="button"
            ...
            <label className="block text-sm font-semibold">
              ...
              <input
                ...
                placeholder="Ej. Derivadas en 60 segundos"
              />
            </label>
            ...
            <label className="block text-sm font-semibold">
              ...
              <select
                required
                value={subject}
                onChange={(event) => setSubject(event.target.value as Exclude<Subject, "Todo">)}
            ...
            <label className="block text-sm font-semibold">
              ...
              <textarea
                ...
                placeholder="¿Qué aprenderá la comunidad?"
              />
            </label>

            <div className="rounded-2xl border border-dashed border-[#3B82F6]/40 bg-[#DBEAFE] px-4 py-7 text-center text-sm text-[#1F2937]/60">
              Previsualización de {type === "foto" ? "foto" : "video"} para la demo
            </div>
            <button className="w-full rounded-full bg-[#FBBF24] px-5 py-3 font-semibold text-[#1F2937]" type="submit">
              Publicar
            </button>
          </form>
        )}
